...
const resources:MapPlacedResource[] = twinSnapshot.resources.filter(item=>item.type==='machine'||item.type==='home').map(item=>({ id:item.id,resourceType:resourceType(item.type),pointId:item.boundPoint??'未绑定',x:item.position.x,y:item.position.y,approach:'0° · 向东' }))
