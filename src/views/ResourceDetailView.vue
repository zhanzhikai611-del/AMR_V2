...
<template>
  <section v-if="item" class="resource-page detail-page">
    <header class="resource-page__header"><div><button class="back-link" @click="router.back()">← 返回{{ kind === 'amr' ? ' AMR 管理' : '设备管理' }}</button><p class="page-eyebrow">RESOURCE DETAIL</p><h1>{{ item.id }}</h1></div><button class="resource-primary-action">编辑资料</button></header>
    <template v-if="kind === 'amr' && 'ip' in item">
      <section class="detail-panel"><header>AMR 信息</header><dl class="detail-grid"><div><dt>名称</dt><dd>{{ item.name }}</dd></div><div><dt>IP 地址</dt><dd class="type-data">{{ item.ip }}</dd></div><div><dt>所属型号</dt><dd>{{ item.model }}</dd></div><div><dt>底盘类型</dt><dd>{{ item.chassis }}</dd></div><div><dt>初始点位</dt><dd>{{ item.initialPoint }}</dd></div><div><dt>额定载荷</dt><dd>{{ item.ratedLoad }}</dd></div><div><dt>运行状态</dt><dd><span class="asset-status" :class="item.tone">{{ item.status }}</span></dd></div><div><dt>当前电量</dt><dd>{{ item.battery }}%</dd></div></dl></section>
      <section class="detail-panel"><header>服务设备</header><div class="scope-detail"><div><span>当前服务范围</span><p><i v-for="id in item.serviceDevices" :key="id">{{ id }}</i></p></div></div></section>
    </template>
    <template v-else-if="'type' in item">
      <section class="detail-panel"><header>设备信息</header><dl class="detail-grid"><div><dt>设备名称</dt><dd>{{ item.name || item.label }}</dd></div><div><dt>设备类型</dt><dd>{{ item.type }}</dd></div><div><dt>设备组</dt><dd>{{ item.group || '—' }}</dd></div><div><dt>绑定点位</dt><dd>{{ item.boundPoint || '—' }}</dd></div><div><dt>启用状态</dt><dd>{{ item.enabled === false ? '停用' : '启用' }}</dd></div><div><dt>连接状态</dt><dd>{{ item.connected === false ? '离线' : '在线' }}</dd></div></dl></section>
      <section class="detail-panel"><header>服务 AMR</header><div class="scope-detail"><div><span>已建立服务关系</span><p><i v-for="amr in serviceAmrs" :key="amr.id">{{ amr.id }}</i></p></div></div></section>
    </template>
  </section>
</template>
