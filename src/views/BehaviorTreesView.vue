...
const query = ref('')
const kind = ref<'all' | 'behavior' | 'subtree'>('all')
...
onMounted(async () => { trees.value = await getBehaviorTrees() })
</script>
  ...
  <section class="resource-page">
    ...
    <div class="behavior-list-toolbar">
      <div class="behavior-kind-tabs">
        <button :class="{ active: kind === 'all' }" @click="kind = 'all'">全部 <b>{{ trees.length }}</b></button>
      ...
      <select v-model="status"><option>全部状态</option><option>已发布</option><option>草稿</option></select>
    </div>

    <div class="resource-table-wrap">
      <table class="resource-table">
        <thead><tr><th>编号</th><th>类型</th><th>名称</th><th>流程摘要</th><th>版本</th><th>引用</th><th>状态</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
    ...
    <div v-if="createOpen" class="modal-backdrop" @click.self="createOpen = false">
      <section class="create-dialog">
        ...
        <label v-if="createKind === 'behavior'">匹配任务类型<select><option>成品转运</option><option>线边补料</option><option>空箱回收</option></select></label>
