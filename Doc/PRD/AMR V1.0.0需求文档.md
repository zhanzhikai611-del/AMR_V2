...
| 功能说明 | 「AMR 信息」包含基本信息、即時信息、服務範圍編輯和接單狀態控制。 |
...
1. 進入「AMR 信息」或切換 AMR 時，載入該 AMR 目前已生效的服務範圍。
...
6. 點擊「保存范围」後提交變更；執行中的 AMR 亦可儲存，變更僅影響後續派單，不改變當前任務及其請求設備。儲存期間避免重複操作；儲存成功後，以最新結果更新 AMR 信息及全局逻辑地图。
...
3. 同一 AMR 同一时刻只能归入一个状态，冲突优先级为“异常 > 充电 > 运行 > 等待 > 空闲”。离线、暂停接单、缺失心跳且无法确认状态的时间不进入有效统计时长。
...
| 空闲 | AMR 在线且可运行，没有活动任务、等待原因或充电行为。 |
| 等待 | AMR 在线且可运行，但因待调度、资源锁、设备握手或交通资源等原因暂未执行。 |
| 充电 | AMR 处于充电状态，以充电状态上报或充电会话区间为准。 |
| 异常 | AMR 因故障或阻断性报警不能正常执行；与其他状态重叠时优先归入异常。 |

每类状态的绝对时长为所有统计 AMR 对应状态区间的秒数之和；占比公式为 `该状态时长 ÷ 全部状态有效时长之和 × 100%`。五类状态互斥且合计 100%。离线、暂停接单及无法确认状态的区间不显示在五类状态中，也不进入分母。
