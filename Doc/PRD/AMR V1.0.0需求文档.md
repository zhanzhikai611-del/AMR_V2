...
| 版本 | 日期 | 修訂內容 | 狀態 |
| --- | --- | --- | --- |
| V1.0.0 | 2026-08-24 | 重整实时监控為「整体页面」與「车辆详情页」兩個模組，補充點擊 AMR 或資源服務編號標籤觸發同一車輛選擇與詳情聯動。 | 討論稿 |
...
| 模組 | 本版深度 | 說明 |
| --- | --- | --- |
| 实时监控 | 完整 | 分為「实时监控整体页面」與「车辆详情页」兩個模組；车辆详情可由 AMR 或 CNC/HOME 左上角的车辆编号标签触发。 |
...
| 车辆详情页（右侧面板） | 點擊 AMR 或服务车辆编号标签後顯示 | 由右側開啟，提供「任务执行」與「车辆信息」兩個子模組。 |
...
车辆详情页以实时监控右側面板呈現，不進行頁面跳轉；其選中狀態、服務範圍和任務路線與全局逻辑地图共用同一份上下文。
